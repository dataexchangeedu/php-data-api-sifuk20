        ...
        if (null === $data) {
            ...
            if (strrpos($inner, ",") !== false) {
                ...
                foreach ($data as $key => $value) {
                    $deserialized[$key] = self::deserialize($value, $subClass, $httpHeaders);
                }
            }
            return $deserialized;
            ...
            $values = [];
            foreach ($data as $key => $value) {
                $values[] = self::deserialize($value, $subClass, $httpHeaders);
            }
            return $values;
        } elseif ($class === 'object') {
            settype($data, 'array');
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $subClass = '\DataExchange\SIFUK20\Models\\' . $data->{$discriminator};
            ...
            foreach ($instance::swaggerTypes() as $property => $type) {
                ...
                $prop = $instance::attributeMap()[$property];
                ...
                if (isset($data->{$prop})) {
                    ...
                    $attr = substr($prop, 1);
                    if (!isset($data->attributes()->{$attr})) {
                        continue;
                    }
                    $instance->$propertySetter((string) $data->attributes()->{$attr});
                }
            }

            if ($class::IS_ARRAY && !is_null($class::ITEMS)) {
                $subClass = '\DataExchange\SIFUK20\Models\\' . $class::ITEMS;
                foreach ($data->children() as $child) {
                    $instance[] = self::deserialize($child, $subClass, $httpHeaders);
                }
            }
